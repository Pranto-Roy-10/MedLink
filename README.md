...
## Latest Updates (May 7, 2026) — Version 2.1

- **Real-time Socket.IO chat:** Full WebSocket-based messaging endpoints and client handlers for low-latency chat in addition to existing polling.
- **Direct Asymmetric Encryption Demo:** `encrypt-demo` route demonstrating pure RSA (direct asymmetric) encryption without symmetric shortcuts.
- **Key Rotation:** `rotate-keys` UI and API to rotate RSA/ECC keys, re-encrypt data, and mark old keys revoked.
- **Prescription & Referral Enhancements:** Prescriptions now use direct RSA encryption for patient delivery; referrals include SHA-256 hashing and HMAC verification with audit trail.
- **Admin Attack Simulation:** `admin/simulate-attack/<id>` simulates tampering to demonstrate HMAC integrity checks and tamper detection.
- **Improved 2FA & Email OTPs:** 6-digit OTP delivery via SMTP with clearer error handling and logging.
- **Sample Data & Initialization:** `init_sample_data()` now seeds RSA/ECC keys, sample messages, referrals and documents for demo/testing.
- **Server port standardized to 5001** (socketio.run uses port 5001 in the current app entrypoint).
- Documentation updated across files (`DOCUMENTATION.md`, `PROJECT_STATUS.txt`) to reflect these changes.
